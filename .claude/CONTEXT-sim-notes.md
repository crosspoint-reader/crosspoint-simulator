...
- **TODO: fork the firmware (deliberately deferred — user decision 2026-07-31).** Firmware feature changes currently ship as vendored patches in `firmware-patches/`, applied by CI to the pinned upstream commit (`CROSSPOINT_FIRMWARE_PIN` in [cmake/CrossPointSources.cmake](../cmake/CrossPointSources.cmake)) before building. That is fine for a small patch stack, but it will not scale: patches conflict as they accumulate, rebasing them across pin bumps is manual, and there is no shared history for the feature work. The eventual fix is a real fork at `natebunnyfield/crosspoint-reader` with feature branches over the pin. Two things blocked doing it from an agent session: the GitHub MCP cannot fork an out-of-scope repo, and repo creation returns 403 for the integration — so the fork must be created in the browser (github.com → crosspoint-reader/crosspoint-reader → Fork), then attached to a session via `add_repo` with push access. After forking: move the vendored patches to real commits on a fork branch, point the TestFlight iOS workflow's firmware checkout at the fork (needs a `firmware_repo` input — currently hardcoded to upstream), and retire `firmware-patches/`.
